...
def send_data():
	try:
		histogram_matrix = np.loadtxt("histogram")
		histogram_matrix = histogram_matrix[~np.all(histogram_matrix[:,1:] == 0, axis=1)]
	except Exception:
	...
	for y in range(1,columns):
		data,data_len = process_data(histogram_matrix,y)
		try:
			r = 0
