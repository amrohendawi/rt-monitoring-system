...
int cpu_numb=0, cores_done=0;
unsigned int priority=90,duration=5;
unsigned int latency_array_size = 1000;
long ** latency_array;
...
void *task(void *vargp){
    ...
    for(int j=0; j<1000; j++){
        ...
        // else printf("time taken %u\t",time_taken);
        // if(j%10 == 0)
        //     printf(" latency at %d has %d \n",time_taken,latency_array[*x][time_taken]);
    }
    cores_done++;
    if(cores_done == cpu_numb){
