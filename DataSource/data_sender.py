...
URL = os.environ.get('TARGET_URL', 'http://0.0.0.0:4545/writereaddata')
...
def send_data():
	try:
		histogram_matrix = np.loadtxt("histogram.txt")
		histogram_matrix[~np.all(histogram_matrix[:,1:] == 0, axis=1)]
	except Exception:
